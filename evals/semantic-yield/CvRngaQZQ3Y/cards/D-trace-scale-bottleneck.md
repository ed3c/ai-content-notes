...
- **獨特特徵**：
  - Scale cost：同一 trace可能被多個 judges、queries 與 iterations 重讀。
  - Context saturation：完整軌跡無法穩定放入單一 context；compaction 又可能刪除 decision-relevant state。
...
- **Typed Links**：ROOT ← [[N-autonomy-trace-mining]] · FLOW → [[P-trace-cycle]] · FLOW → [[C-model-harness-task-fit]]
