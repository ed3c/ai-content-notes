...
This document is this repository's promotion policy for turning frontier content, conversation context, repeated failures, and review knowledge into reusable domain context **without building a new framework for every source**.

It is not the architecture-compiler product line's decision record. That line left this repository under [#83](https://github.com/ed3c/ai-content-notes/issues/83); its successor owner is `ed3c/skill-concerns#19`, and its merged artifacts are `ed3c/skill-concerns#36` (merge `832415ea3ca1fb61a8b974032246ff27b25576c1`) and `ed3c/noodles#255` (merge `fe05ed440e90afb7c44b487c1069b186dd22f4e2`). Do not re-index that material here.
...
## Reference fixtures live elsewhere

This policy carries no reference corpus. The fixture that demonstrates these principles, and the promotion decisions taken against it, left this repository under #83 and are owned by `ed3c/skill-concerns#36` and `ed3c/noodles#255` at the merges named above.

Landing a fixture here again would re-couple the product line that #83 separated.
...
3. this file;
4. the exact source/card artifacts involved;
5. downstream FeatureMap/Spatial Loop contracts only if the Promotion Gate requires escalation.

Issue #75 is `CLOSED`; it is history, not status. Its residue landed as `tests/test_convergence_ceiling.py`, and the escalation machinery it once named is owned by `ed3c/skill-concerns#19`. Read a closed issue for provenance only — this repository's state is what `main` carries.
