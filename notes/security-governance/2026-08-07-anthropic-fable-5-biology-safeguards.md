...
path: "notes/security-governance/2026-08-07-anthropic-fable-5-biology-safeguards.md"
migration:
  from_repository: ed3c/openwiki-ablation
  from_path: "ai-content-notes/notes/security-governance/2026-08-07-anthropic-fable-5-biology-safeguards.md"
...
- **漏洞利用 (Exploit/How)**：  
  1. 每個 request 記錄 `risk_class`, `classifier_score`, `selected_model`, `fallback_reason`, `latency_ms`, `task_success`。  
  2. 依 domain / profession / locale 切 fallback rate；特別監控 clinical、education、research。  
  3. 建 `false_positive_review_queue`：使用者重試、改寫後成功、人工覆核為 benign 的樣本自動進 queue。  
  4. 新 classifier 上線前跑 regression：`harmful_recall >= baseline` 且 `benign_fallback_rate` 顯著下降。  
